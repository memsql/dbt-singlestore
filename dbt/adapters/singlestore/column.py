...
class SingleStoreColumn(Column):
    ...
    @property
    def _normalized_dtype(self) -> str:
        t = (self.dtype or "").strip().lower()
        if "(" in t:
            t = t.split("(", 1)[0]
        if t == "integer":
            t = "int"
    ...
    def is_integer(self) -> bool:
        return self._normalized_dtype in self.INTEGER

    def is_float(self) -> bool:
        return self._normalized_dtype in self.FLOAT

    def is_numeric(self) -> bool:
        return self._normalized_dtype in self.DEC

    def is_string(self) -> bool:
        return self._normalized_dtype in self.STRING

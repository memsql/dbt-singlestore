...
select * from seed_expected

"""


seeds__expected_sql = """
create table seed_expected (
seed_id INTEGER,
first_name TEXT,
email TEXT,
ip_address TEXT,
...
class SingleStoreSimpleSeedMixin:
...
class TestSeedConfigFullRefreshOff(SingleStoreSimpleSeedMixin, BaseSeedConfigFullRefreshOff):
    pass


class TestSeedCustomSchema(SingleStoreSimpleSeedMixin, BaseSeedCustomSchema):
    pass


class TestSeedWithUniqueDelimiter(SingleStoreSimpleSeedMixin, BaseSeedWithUniqueDelimiter):
    pass


class TestSeedWithWrongDelimiter(SingleStoreSimpleSeedMixin, BaseSeedWithWrongDelimiter):
    def test_seed_with_wrong_delimiter(self, project):
        seed_result = run_dbt(["seed"], expect_pass=False)
        msg = seed_result.results[0].message.lower()
        assert "error in your sql syntax" in msg
    pass


class TestSeedWithEmptyDelimiter(SingleStoreSimpleSeedMixin, BaseSeedWithEmptyDelimiter):
    pass


class TestSeedParsing(SingleStoreSimpleSeedMixin, BaseSeedParsing):

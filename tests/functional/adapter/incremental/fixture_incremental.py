...
# Models
#
# We override all models because we change every usage of the CAST commands with :> operator
...
SELECT id,
        (field1 :> {{string_type}}) as field1,
        (field3 :> {{string_type}}) as field3, -- to validate new fields
        (field4 :> {{string_type}}) AS field4 -- to validate new fields
...
SELECT id,
       (field1 :> {{string_type}}) as field1,
       (field3 :> {{string_type}}) as field3,
       (field4 :> {{string_type}}) as field4
FROM source_data WHERE id NOT IN (SELECT id from {{ this }} )
...
select id
       ,(field1 :> {{string_type}}) as field1
       ,(field2 :> {{string_type}}) as field2
       ,((CASE WHEN id <= 3 THEN NULL ELSE field3 END) :> {{string_type}}) AS field3
       ,((CASE WHEN id <= 3 THEN NULL ELSE field4 END) :> {{string_type}}) AS field4
...
select id,
       (field1 :> {{string_type}}) as field1,
       ((CASE WHEN id >  3 THEN NULL ELSE field2 END) :> {{string_type}}) AS field2,
       ((CASE WHEN id <= 3 THEN NULL ELSE field3 END) :> {{string_type}}) AS field3,
       ((CASE WHEN id <= 3 THEN NULL ELSE field4 END) :> {{string_type}}) AS field4

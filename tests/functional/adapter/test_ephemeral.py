...
class TestEphemeralMulti(SqlGlobalOverrideMixin, BaseEphemeralMulti):
    BASE_TEST_CLASS = BaseEphemeralMulti
    SQL_GLOBAL_OVERRIDES = {
        "models__base__base_sql": models__base__base_sql,
    }
    ...
    def test_ephemeral_multi(self, project):
        ...
        expected_sql = """
        create view `dbt_test`.`double_dependent__dbt_tmp` as
...
class TestEphemeralNested(BaseEphemeralNested):
    def test_ephemeral_nested(self, project):
        ...
        sql_file = re.sub(r"\d+", "", sql_file)
        expected_sql = """
        create view `dbt_test`.`root_view__dbt_tmp` as
        with __dbt__cte__ephemeral_level_two as (
        select * from `dbt_test`.`source_table`
...
class TestEphemeralErrorHandling(SqlGlobalOverrideMixin, BaseEphemeralErrorHandling):
    BASE_TEST_CLASS = BaseEphemeralMulti
    SQL_GLOBAL_OVERRIDES = {
        "ephemeral_errors__base__base_sql": ephemeral_errors__base__base_sql,
    }
    pass
